...
- **专门设置页** —— 点击 popup 的「更多设置」打开完整的设置标签页:预制与自定义主题管理、二维位置调节、自定义 HTML 模板(支持 `{{hh}} {{mm}} {{ss}}` 占位符与 SVG 装饰等任意结构)、开源地址
...
> 扩展弹窗(popup):顶部为时钟实时预览,下方为「快速调整」子集——外观(字号 / 粗体 / 文字颜色 / 背景颜色 / 透明度)与显示(仅全屏显示 / 常驻显示)。预制主题、位置、自定义 HTML 模板等完整功能在「⚙ 更多设置」打开的设置页中。

![BiClock 设置页](./options-shot.png)

> 设置页(options):三栏布局——左侧分区导航、中间滚动控制面板(主题网格、外观与显示、自定义 HTML 模板含 `{{hh}} {{mm}} {{ss}} {{time}}` 占位符与示例、关于),右侧为还原 B 站播放器外壳的实时预览,可直接拖动时钟调整位置。
...
4. 想换位置?直接按住时钟拖到任意位置即可;或点击工具栏的扩展图标,在 popup 中快速调整外观与显示模式;需要主题、自定义 HTML 模板等更多设置时,点击 popup 顶部的「⚙ 更多设置」打开完整设置页
