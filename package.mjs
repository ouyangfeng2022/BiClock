...
// 用 Windows 内置的 bsdtar (libarchive) 生成 zip。
// 比 PowerShell 的 Compress-Archive 更标准：生成的归档用正斜杠路径分隔符，
// 符合 zip 规范，被所有扩展商店正常识别。
// `-a` 让 bsdtar 根据输出文件扩展名 (.zip) 自动选 zip 格式；
// 在暂存目录里传 `.` 让内容直接落在归档根目录。
