...
- `biclock.js` is tightly coupled to Bilibili's current player DOM. It queries specific class names that can change when Bilibili ships a player update:
  ...
  - Attribute `data-ctrl-hidden` on `.bpx-player-container` — `"false"` means controls visible. Combined with `data-screen="full"`, the clock shows; `shouldShow()` is the single gatekeeper.
...
- A `MutationObserver` watches attribute changes on `.bpx-player-container`; the 1-second `setInterval` is started only when `shouldShow()` is true (fullscreen + controls visible) and stopped otherwise. `stopTimer()` also detaches the clock node from the DOM so it doesn't linger in non-fullscreen modes. Don't introduce a second always-running interval — keep it tied to `shouldShow()`.
