...
- `updateClock()` re-inserts the clock node on every tick. The clock node is reused (same reference), so this moves rather than duplicates it; if you change this, preserve the "single reused node" invariant to avoid stacking duplicates. **Mount point depends on display mode:** in mouse-triggered mode it is inserted after `.bpx-player-top-left` (inside the top bar, so it fades out with the bar); in always-on mode it is appended to `.bpx-player-container` and switched to `position: fixed`, because Bilibili hides the entire top-bar subtree when controls auto-hide, which would otherwise take the clock with it even though `shouldShow()` is still true.
